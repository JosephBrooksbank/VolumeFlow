...
class VolumeFlow(FlowLauncher):
    ...
    def query(self, query):
        results=[]
        if query == "speakers":
            subprocess.run(f"nircmd.exe setdefaultsounddevice HIFI")
        if query == "headset":
            subprocess.run(f"nircmd.exe setdefaultsounddevice Headset")
        elif query == "up":
            ...
            results.append({
                    ...
                    "JsonRPCAction": {"method": "speakers", "parameters":[]  }
                    })
            results.append({
                    "Title": "Use Headset",
                    # "IcoPath": f"Images/up.png",
                    "JsonRPCAction": {"method": "headset", "parameters":[]  }
                    })
            results.append({
                    "Title": "Volume up",
                    "IcoPath": f"Images/up.png",
